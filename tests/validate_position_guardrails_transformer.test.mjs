...
test("the Position Guardrails transformer is mirrored for deployment", async () => {
  const [sourceText, deployed] = await Promise.all([
    readFile(new URL("position-guardrails-transformer.js", root), "utf8"),
    readFile(new URL("deploy-site/position-guardrails-transformer.js", root), "utf8"),
  ]);
  assert.equal(deployed, sourceText);
});
