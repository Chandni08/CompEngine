...
test("recommendations carry owners, options, final gates, effort, internal evidence, and magnitude", () => {
  ...
  intelligence.recommendations.forEach((decision) => {
    ...
    assert.ok(decision.decisionDue);
...
test("web and exports use the same snapshot and inference vocabulary", () => {
  ...
  assert.match(appSource, /Alternative reading/);
  assert.match(appSource, /Falsifier \/ signal to watch/);
  assert.match(appSource, /Business impact.*Unquantified/);
  assert.doesNotMatch(appSource, /\$\{breakdown\.total\}\/100/);
  assert.match(pptxSource, /BUSINESS IMPACT · UNQUANTIFIED/);
