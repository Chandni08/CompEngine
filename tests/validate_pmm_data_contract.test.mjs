...
test("the PMM contract runtime is loaded by the local workspace", async () => {
  const [sourceContract, sourceIndex] = await Promise.all([
    readFile(new URL("../pmm-data-contract.js", import.meta.url), "utf8"),
    readFile(new URL("../index.html", import.meta.url), "utf8"),
  ]);
  assert.match(sourceContract, /PmmDataContract/);
