...
test("hierarchical targeting exposes Waters product, application, buying situation, geography, and buyer-role controls inside the eight-section spine", () => {
  for (const id of ["segmentFilter", "pmmWatersProductFilter", "pmmApplicationFilter", "pmmBuyingSituationFilter", "geoFilter", "pmmBuyerRoleFilter"])
    assert.match(index, new RegExp(`id="${id}"`));
  assert.equal((index.match(/class="panel pmm-primary-section"/g) || []).length, 8);
  assert.match(app, /market: filters\.segment\.value/);
  assert.match(app, /watersProduct: selectedWatersProduct\?\.product/);
  assert.match(app, /application: state\.marketingTargeting\.application/);
  assert.match(app, /buyingSituation: state\.marketingTargeting\.buyingSituation/);
  assert.match(app, /geography: filters\.geo\.value/);
  assert.match(app, /buyerRole: state\.marketingTargeting\.buyerRole/);
  assert.doesNotMatch(index, /id="pmmTargetingContext"/);
});

test("Waters product filtering uses canonical catalog IDs and explicit comparison mappings", async () => {
  const comparisons = JSON.parse(await readFile(new URL("data/product_comparisons.json", root), "utf8"));
  assert.ok(comparisons.watersSystems.length >= 1);
  assert.ok(comparisons.watersSystems.every((product) => product.id && product.product && /^https?:/.test(product.sourceUrl)));
  assert.match(app, /state\.productComparisons\?\.watersSystems/);
  assert.match(app, /comparison\?\.closestWatersId === selectedWatersProduct\.id/);
  assert.match(app, /technicalProfile\?\.watersId === selectedWatersProduct\.id/);
  assert.match(app, /\.filter\(\(context\) => !selectedWatersProduct \|\| context\.productComparisonMatched\)/);
  assert.match(app, /pmmWatersProductCode\(governingPosition\.targeting\.watersProductId\)/);
});
...
test("every hierarchical change re-derives governed downstream artifacts from one model", () => {
  ...
  assert.match(documentContract, /Market → Waters product → Application\/workflow → Buying situation → Geography → Buyer role/);
});
