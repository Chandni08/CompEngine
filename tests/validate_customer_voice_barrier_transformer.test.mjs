...
test("the Customer Voice barrier module is mirrored for deployment", async () => {
  const [source, deployed] = await Promise.all([
    readFile(new URL("customer-voice-barrier-transformer.js", root), "utf8"),
    readFile(new URL("deploy-site/customer-voice-barrier-transformer.js", root), "utf8"),
  ]);
