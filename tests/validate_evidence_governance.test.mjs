...
test("the governance runtime and entry point are mirrored for deployment", async () => {
  const [sourceRuntime, deployRuntime, sourceIndex, deployIndex] = await Promise.all([
    readFile(new URL("evidence-governance.js", root), "utf8"),
    readFile(new URL("deploy-site/evidence-governance.js", root), "utf8"),
    readFile(new URL("index.html", root), "utf8"),
    readFile(new URL("deploy-site/index.html", root), "utf8"),
  ]);
  assert.equal(deployRuntime, sourceRuntime);
