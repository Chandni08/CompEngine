# Remaining evidence gaps

Generated: `2026-08-02T20:38:41+00:00`

This is the concise roll-up of every non-verified claim retained in the current audit denominator. The claim-level reason, source URL, and disposition remain in `per_claim_appendix.csv`.

## Partially supported — 192

| Claim family | Count |
| --- | ---: |
| Signal card | 65 |
| Customer voice | 41 |
| Publication count | 32 |
| Market/application source | 29 |
| Historical product catalog | 7 |
| Journal source | 7 |
| Conference intelligence | 5 |
| Competitor launch | 3 |
| Application note | 2 |
| Decision | 1 |

## UNREACHABLE — 450

| Claim family | Count |
| --- | ---: |
| SEC filing | 177 |
| Historical product catalog | 113 |
| Signal card | 46 |
| Technical comparison | 32 |
| Customer voice | 21 |
| Scientific publication | 19 |
| Launch comparison | 10 |
| Journal source | 9 |
| Application trend | 5 |
| Application note | 4 |
| Corporate move | 4 |
| Source health | 3 |
| Competitor launch | 2 |
| Decision | 2 |
| Product evidence | 1 |
| Conference intelligence | 1 |
| Market/application source | 1 |

## Unsupported/Hallucinated — 160

| Claim family | Count |
| --- | ---: |
| Product evidence | 157 |
| Source health | 3 |

## Contradicted — 13

| Claim family | Count |
| --- | ---: |
| Source health | 12 |
| Corporate move | 1 |

## Link access limitations

- 1071 URLs were reachable and semantically valid.
- 850 URLs were blocked by access controls, rate limits, or bot challenges.
- 0 URLs were broken.
- 0 URLs were semantic mislinks.

Blocked URLs remain non-healthy and are listed individually in `broken_mislink_ledger.csv`. Contradicted and unsupported baseline claims remain visible for audit trace even after removal from current UI surfaces.
