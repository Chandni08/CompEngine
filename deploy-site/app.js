...
const state = {
  ...
  filingInsights: null,
  patentInsights: null,
  leadershipProfiles: null,
  leadershipPeople: null,
  hiringPatterns: null,
  customerVoice: null,
  refreshStatus: null,
  linkHealth: null,
...
let navigationTransitionToken = 0;
let sectionNavRefreshFrame;
let navigationFocusTarget;
let sectionNavigatorTargets = [];
let sidebarViewportQuery;
const MINIMUM_SECTION_ANCHOR_OFFSET = 96;

function sectionAnchorOffset() {
  const configuredOffset = Number.parseFloat(window.getComputedStyle(document.documentElement)
    .getPropertyValue("--section-anchor-offset")) || 0;
  return Math.max(MINIMUM_SECTION_ANCHOR_OFFSET, configuredOffset);
}
...
function releaseActiveSectionNavLock(transitionToken, { onlyIfSettled = false } = {}) {
  if (transitionToken !== navigationTransitionToken || !navigationSelectionLockId) return;
  const target = byId(navigationSelectionLockId);
  const atDocumentEnd = window.scrollY + window.innerHeight >= document.documentElement.scrollHeight - 2;
  const targetIsAligned = target
    ? Math.abs(target.getBoundingClientRect().top - sectionAnchorOffset()) <= 8
    : false;
  if (onlyIfSettled && !targetIsAligned && !atDocumentEnd) return;
  window.clearTimeout(navigationSelectionLockTimer);
  navigationSelectionLockId = "";
  scheduleSectionNavRefresh();
}

function lockActiveSectionNav(targetId, transitionToken, duration = 2200) {
  window.clearTimeout(navigationSelectionLockTimer);
  ...
  navigationSelectionLockTimer = window.setTimeout(() => {
    ...
    releaseActiveSectionNavLock(transitionToken);
  }, duration);
}
...
function updateSectionAnchorOffset() {
  ...
  const anchorOffset = Math.max(
    MINIMUM_SECTION_ANCHOR_OFFSET,
    stickyHeight + stickySidebarHeight + 24,
  );
  document.documentElement.style.setProperty("--section-anchor-offset", `${anchorOffset}px`);
}
...
function navigateToDashboardSection(targetId, { behavior = "smooth", updateHistory = true } = {}) {
  ...
  const transitionToken = ++navigationTransitionToken;
  const shouldStopPriorScroll = Boolean(navigationSelectionLockId && navigationSelectionLockId !== targetId);
  if (shouldStopPriorScroll) window.scrollTo({ top: window.scrollY, behavior: "auto" });
  revealDashboardSection(target);
  lockActiveSectionNav(targetId, transitionToken, 0);
  if (updateHistory) window.history.replaceState(null, "", `#${targetId}`);
  window.requestAnimationFrame(() => {
    if (transitionToken !== navigationTransitionToken) return;
    updateSectionAnchorOffset();
    window.requestAnimationFrame(() => {
      if (transitionToken !== navigationTransitionToken) return;
      scrollToDashboardSection(target, behavior);
      lockActiveSectionNav(targetId, transitionToken, behavior === "auto" ? 250 : 2200);
      if (behavior === "auto") {
        window.requestAnimationFrame(() => releaseActiveSectionNavLock(transitionToken, { onlyIfSettled: true }));
      }
    });
  });
  return true;
...
function setupSectionNavigator() {
  ...
  window.addEventListener("scrollend", () => {
    releaseActiveSectionNavLock(navigationTransitionToken, { onlyIfSettled: true });
  });
...
function setupLeadershipProfileNavigation() {
  byId("leadershipProfiles").addEventListener("click", (event) => {
    const personTrigger = event.target.closest("button[data-leadership-person-select]");
    if (personTrigger) {
      state.activeLeadershipPersonByCompetitor[personTrigger.dataset.competitor] = personTrigger.dataset.leadershipPersonSelect;
      renderLeadershipBehaviorProfiles();
      return;
    }
    const personAction = event.target.closest("button[data-leadership-person-action]");
    if (personAction) {
      moveLeadershipPersonSlider(personAction.dataset.competitor, personAction.dataset.leadershipPersonAction);
      return;
    }
    const trigger = event.target.closest("button[data-leadership-select]");
    if (!trigger) return;
    state.activeLeadershipCompetitor = trigger.dataset.leadershipSelect;
    renderLeadershipBehaviorProfiles();
  });
  byId("leadershipProfiles").addEventListener("keydown", (event) => {
    const slider = event.target.closest("[data-leadership-person-slider]");
    if (!slider || !["ArrowLeft", "ArrowRight"].includes(event.key)) return;
    event.preventDefault();
    moveLeadershipPersonSlider(slider.dataset.competitor, event.key === "ArrowLeft" ? "previous" : "next");
  });
}

function setupHiringPatternNavigation() {
  byId("hiringPatterns").addEventListener("click", (event) => {
    const trigger = event.target.closest("button[data-hiring-competitor]");
    if (!trigger) return;
    state.activeHiringCompetitor = trigger.dataset.hiringCompetitor;
    renderHiringPatterns();
  });
}
...
function setCustomerVoiceTab(tabName) {
  ...
  tabList.classList.remove("has-positioning-tab");
  ...
  document.querySelectorAll("[data-customer-voice-panel]").forEach((panel) => {
    ...
    panel.hidden = marketingOnly || panel.dataset.customerVoicePanel !== activeTab;
  });
}
...
function marketingComparatorRead(launch, comparison) {
  ...
  const sourceRead = marketingComparatorSourceRead(comparison.pmRead);
  return `PMM should treat ${competitor}'s offer as a buying narrative, not only a feature comparison. ${sourceRead} Translate the defensible Waters advantage into role-specific proof; keep unverified superiority, uptime, and outcome claims out of field materials.`;
}
...
function marketingProductShareStrategyMarkup(launch, waters, comparison, featureProfile) {
  ...
  const segments = itemMarketSegments(launch).filter((segment) => segment && segment !== "All").join(" / ") || "qualified target";
...
function renderComparisonBody() {
  ...
  const watersMeaning = state.view === "Marketing" ? marketingComparatorRead(launch, comparison) : comparison.pmRead;
  ...
  byId("comparisonBody").innerHTML = `
    ...
    <section class="comparison-positioning">
      <div>
        <h4>${state.view === "Marketing" ? "PMM Read" : "What This Means for Waters"}</h4>
      ...
      <div>
        <h4>${state.view === "Marketing" ? "Evidence-Backed Waters Position" : "How to Position Waters"}</h4>
...
function pmmProductComparatorClaimTransformation(governingPosition) {
  ...
  const candidates = transformation.candidates.map((candidate) => positionGuardrailsTransformer.flagDownstreamClaim(candidate, {
    exclusions: governingPosition.exclusionRecords,
    textField: "claimText",
    evidenceField: "supportingEvidence",
  }));
  const candidateById = new Map(candidates.map((candidate) => [candidate.id, candidate]));
  const transformedGaps = transformation.gapQueue.map((candidate) => ({
    ...candidate,
    ...(candidateById.get(candidate.id) || {}),
    fieldUsable: false,
  }));
  const allClaimControlClaims = candidates.filter((candidate) => candidate.status === "supported" && candidate.approvalState !== "blocked");
  const claimControlClaims = pmmGovernedRecords(allClaimControlClaims);
  gapQueue.splice(0, gapQueue.length, ...transformedGaps);
  return {
    ...transformation,
    candidates,
    allClaimControlClaims,
    claimControlClaims,
    fieldUsableClaims: claimControlClaims.filter((claim) => claim.fieldUsable === true),
    gapQueue: [...gapQueue],
  };
}
...
function persistRoleView(selectedView = state.view) {
  if (!validRoleViews.has(selectedView)) return;
  try { localStorage.setItem(roleViewStorageKey, selectedView); } catch { /* device-local persistence unavailable */ }
  const url = new URL(window.location.href);
  if (selectedView === "Product") url.searchParams.delete("view");
...
function pmmOverallTrendGuardrailInputs(signals) {
  return overallTrendCandidates()
    .map((candidate) => {
      const overallTrendEvidenceModel = overallTrendEvidence(signals, candidate);
      const groups = overallTrendEvidenceModel.groups.map((group) => ({
        ...group,
        items: pmmGovernedRecords(group.items),
      }));
      ...
      return {
        ...candidate,
        evidence,
        overallTrendFamilyCount: overallTrendEvidenceModel.familyCount,
        overallTrendRecordCount: overallTrendEvidenceModel.recordCount,
        narrative: horizonTrendNarrative(candidate),
      };
    })
    .filter((candidate) => candidate.overallTrendFamilyCount >= 2)
    .sort((left, right) => right.overallTrendFamilyCount - left.overallTrendFamilyCount || right.overallTrendRecordCount - left.overallTrendRecordCount)
    .slice(0, 5);
}

function pmmGoverningPosition(contexts, selectedSwingAttribute = "Swing attribute unresolved — no priority-segment scorecard is available", signals = currentSignals()) {
  if (!positionGuardrailsTransformer) throw new Error("Position Guardrails transformer failed to load");
  const targeting = pmmTargetingSelection();
  const trendInputs = pmmOverallTrendGuardrailInputs(signals);
  const guardrails = positionGuardrailsTransformer.transformOverallTrends({
    trends: trendInputs,
    approvalState: "draft",
  });
  ...
  return {
    ...
    narrativeSpine: guardrails.narrativeSpine,
    primaryValueProposition: guardrails.narrativeSpine.whyWaters || "Gap — Overall Trend Analysis does not contain a Why Waters implication under the active filters.",
    ...
    pointOfDifference: guardrails.evidencePillars[0]?.statement || "Gap — no field-citable Overall Trend Analysis pillar is available under the active filters.",
    selectedSwingAttribute,
    evidencePillars: guardrails.evidencePillars,
    framingOnlyTrends: guardrails.framingOnlyTrends,
    pillarRequirement: guardrails.pillarRequirement,
    exclusions: guardrails.exclusions,
    exclusionRecords: guardrails.exclusionRecords,
    overallTrendInputs: trendInputs,
    fieldCitable: false,
    approvalState: guardrails.approvalState,
    approver: "Approver needed",
    lastReviewedDate: "Review date unavailable",
    evidenceClassification: "Draft synthesis of Overall Trend Analysis; pillars require field-citable backing and remain unapproved",
  };
}
...
function pmmGoverningTrace(governingPosition, localAdaptation) {
  ...
  const exclusionConflicts = positionGuardrailsTransformer.detectExclusionConflicts({
    claimText: local,
    exclusions: governingPosition.exclusionRecords,
  });
  return {
    governingPositionId: governingPosition.id,
    localAdaptation: local,
    ...evaluation,
    status: exclusionConflicts.length ? "contradiction" : evaluation.status,
    message: exclusionConflicts.length
      ? `Position Guardrails conflict — ${exclusionConflicts.map((conflict) => conflict.reason).join(" ")} This language must not activate.`
      : evaluation.message,
    exclusionConflicts,
  };
}
...
function pmmProofPriorities(signals, comparatorClaimTransformation, customerVoiceBarrierTransformation, governingPosition) {
  ...
  const gapCandidates = [
    ...
    })).map((item) => positionGuardrailsTransformer.flagDownstreamClaim(item, {
      exclusions: governingPosition.exclusionRecords,
      textField: "claimText",
      evidenceField: "supportingEvidence",
    }));
  const decisionCandidates = pmmProofDecisionInputs(signals).map((item) => positionGuardrailsTransformer.flagDownstreamClaim(item, {
    exclusions: governingPosition.exclusionRecords,
    textField: "commercialClaim",
    evidenceField: "supportingEvidence",
  }));
  const result = proofPriorityTransformer.aggregateProofPriorities({
    gapQueue: gapCandidates.filter((item) => !(item.guardrailConflicts || []).length),
    decisionItems: decisionCandidates.filter((item) => !(item.guardrailConflicts || []).length),
  ...
  return {
    ...result,
    guardrailBlocked: [...gapCandidates, ...decisionCandidates].filter((item) => (item.guardrailConflicts || []).length),
  };
}
...
function pmmGoverningPositionMarkup(position) {
  return `<article class="pmm-governing-position" data-governing-position-id="${escapeHtml(position.id)}" aria-labelledby="pmmGoverningPositionTitle">
    <header class="pmm-governing-header">
      <div><div class="pmm-eyebrow">Overall Trend Analysis → Canonical PMM Object</div><h4 id="pmmGoverningPositionTitle">Position Guardrails</h4><p>The Why Now / Why Waters spine, citable pillars, and explicit exclusions govern every downstream PMM claim.</p></div>
      ${pmmEvidenceTypeMarkup("inference", pmmApprovalStateLabel(position.approvalState))}
    </header>
    <section class="pmm-narrative-spine" aria-labelledby="pmmNarrativeSpineTitle">
    ...
    <div class="pmm-governing-governance" aria-label="Governing position approval and review state">
      ${pmmStatusMarkup(position.approvalState === "approved" ? "observed" : "unresolved", "Overall approval state", pmmApprovalStateLabel(position.approvalState))}
...
function marketingClaimsProofRows(contexts, governingPosition) {
  ...
  return rows.map((row) => positionGuardrailsTransformer.flagDownstreamClaim(row, {
    exclusions: governingPosition.exclusionRecords,
    textField: "proposedClaimWording",
    evidenceField: "evidenceRecords",
  }));
}
...
function pmmApplyClaimsRegistryToDecisions(decisions, claimRows) {
  return decisions.map((decision) => {
    ...
    return {
      ...
      missingProof: governedClaim.guardrailStatus === "conflict"
        ? `Position Guardrails conflict — this claim must not activate. ${governedClaim.guardrailConflicts.map((conflict) => conflict.reason).join(" ")}`
        : governedClaim.nextRequiredAction,
      claimRegistry: {
        ...
        guardrailStatus: governedClaim.guardrailStatus,
        guardrailConflicts: governedClaim.guardrailConflicts,
      },
    };
  });
}
...
function pmmGuardrailConflictMarkup(claim) {
  const conflicts = claim.guardrailConflicts || [];
  if (!conflicts.length) return "";
  return `<aside class="pmm-guardrail-conflict" role="alert"><strong>POSITION GUARDRAILS CONFLICT · NOT FIELD-USABLE</strong>${conflicts.map((conflict) => `<span>${escapeHtml(conflict.claimText)}</span><small>${escapeHtml(conflict.reason)}</small>`).join("")}</aside>`;
}

function pmmComparatorClaimControlMarkup(supportedClaims = [], queuedGaps = []) {
  const fieldUsableCount = supportedClaims.filter((claim) => claim.fieldUsable === true).length;
  return `<section class="pmm-comparator-claim-control" aria-labelledby="pmmComparatorClaimControlTitle">
    ...
    <div class="pmm-claims-table-wrap"><table class="pmm-comparator-claims-table"><caption class="sr-only">Supported Product Comparator claim candidates</caption><thead><tr><th>Exact wording</th><th>Applicable proof</th><th>Blocked evidence</th><th>Approval state</th><th>Study required before field use</th></tr></thead><tbody>${supportedClaims.length
      ? supportedClaims.map((claim) => `<tr id="${escapeHtml(pmmComparatorClaimTraceAnchorId(claim))}" data-comparator-claim-id="${escapeHtml(claim.id)}" data-claim-status="supported" data-field-usable="${claim.fieldUsable === true}" tabindex="-1">
        ...
        <td><strong>${escapeHtml(claim.guardrailStatus === "conflict" ? "Resolve or withdraw the wording that conflicts with Position Guardrails." : claim.studyRequiredBeforeFieldUse)}</strong><small>${claim.guardrailStatus === "conflict" ? "Field-use gate failed: explicit exclusion conflict." : "Field-use gate: citable proof present, approval is not blocked, and no exclusion conflict is detected."}</small></td>
      </tr>`).join("")
...
function pmmClaimRiskScore(row) {
  ...
  const guardrailConflict = row.guardrailStatus === "conflict" ? 30 : 0;
...
function pmmTargetingBreakReportModel(claimRows, buyingCommittee, positioningDecisions, narratives, adoptionValuePlans = []) {
  ...
  return PmmDataContract.buildTargetingBreakReport({
    ...
    governingTraces: [
      ...
      ...claimRows.filter((claim) => claim.guardrailStatus === "conflict").map((claim) => ({
        status: "contradiction",
        label: `${claim.competitor} Claim Control row`,
        message: `Position Guardrails conflict — ${claim.guardrailConflicts.map((conflict) => conflict.reason).join(" ")}`,
      })),
    ],
...
function pmmDownstreamGuardrailConflicts({ claimRows = [], comparatorClaims = [], proofPriorityClaims = [], positioningDecisions = [], narratives = [] } = {}) {
  const candidates = [
    ...claimRows.map((claim) => ({ claimText: claim.proposedClaimWording, source: "Claim Control registry", conflicts: claim.guardrailConflicts || [] })),
    ...comparatorClaims.map((claim) => ({ claimText: claim.claimText, source: "Product Comparator claim candidate", conflicts: claim.guardrailConflicts || [] })),
    ...proofPriorityClaims.map((claim) => ({ claimText: claim.claimText || claim.commercialClaim, source: "Proof Priority candidate", conflicts: claim.guardrailConflicts || [] })),
    ...positioningDecisions.map((decision) => ({ claimText: decision.counterPosition, source: "Positioning decision", conflicts: decision.governingTrace?.exclusionConflicts || [] })),
    ...narratives.map((narrative) => ({ claimText: narrative.counterPosition, source: `${narrative.competitor} narrative`, conflicts: narrative.governingTrace?.exclusionConflicts || [] })),
  ].filter((candidate) => candidate.claimText && candidate.conflicts.length);
  const seen = new Set();
  return candidates.filter((candidate) => {
    const key = `${candidate.source}|${candidate.claimText}`;
...
function buildMarketingWorkspaceModel(signals) {
  ...
  const provisionalGoverningPosition = pmmGoverningPosition(contexts, undefined, signals);
  ...
  const governingPosition = pmmGoverningPosition(contexts, buyingCommittee.selectedSwingAttribute, signals);
  ...
  const comparatorClaimTransformation = pmmProductComparatorClaimTransformation(governingPosition);
  const customerVoiceBarrierDraft = pmmCustomerVoiceBarrierTransformation();
  const proofPriorities = pmmProofPriorities(governedSignals, comparatorClaimTransformation, customerVoiceBarrierDraft, governingPosition);
  ...
  const competitorPlays = pmmCompetitorIntentSellingMotions(
    governedSignals,
    comparatorClaimTransformation.fieldUsableClaims,
    proofPriorities,
  );
  ...
  governingPosition.downstreamConflicts = pmmDownstreamGuardrailConflicts({
    claimRows,
    comparatorClaims: comparatorClaimTransformation.candidates,
    proofPriorityClaims: proofPriorities.guardrailBlocked,
  ...
  const baseModel = {
    ...
    productComparatorFieldUsableClaims: comparatorClaimTransformation.fieldUsableClaims,
...
function currentNewsroomSignals(signals) {
  return signals
    .filter((signal) => /press release|earnings event announcement|quarterly earnings result/i.test(signal.signalType || "")
      || /newsroom|investor relations news/i.test(signal.sourceName || ""))
    .sort((a, b) => new Date(b.date) - new Date(a.date));
}

function currentCorporateNewsroomSignals(signals) {
  const seen = new Set();
  return [...currentNewsroomSignals(signals), ...currentStrategicSignals(signals)]
    .filter((signal) => {
      const key = signal.id || canonicalEvidenceUrl(signal.sourceUrl || signal.url) || `${signal.competitor}|${signal.date}|${signal.title}`;
      if (seen.has(key)) return false;
      seen.add(key);
...
function currentPatentInsights() {
  const insights = state.patentInsights?.insights || [];
  return insights
    .filter((insight) => inHorizon(insight.publicationDate || insight.date, filters.horizon.value))
    .filter((insight) => filters.segment.value === "All" || (insight.marketSegments || [insight.marketSegment]).includes(filters.segment.value))
    .filter((insight) => technologyMatchesFilter(
      insight.technology,
      filters.technology.value,
...
function moveLeadershipPersonSlider(competitor, direction) {
  const people = leadershipPeopleForCompetitor(competitor);
  if (!people.length) return;
  const activeId = state.activeLeadershipPersonByCompetitor[competitor];
  const activeIndex = Math.max(0, people.findIndex((person) => person.id === activeId));
  const offset = direction === "previous" ? -1 : 1;
  const nextIndex = (activeIndex + offset + people.length) % people.length;
  state.activeLeadershipPersonByCompetitor[competitor] = people[nextIndex].id;
  renderLeadershipBehaviorProfiles();
}
...
function marketingRecommendationFor(rec) {
  const themeKey = recommendationThemeKey(rec);
  const segment = filters.segment.value === "All" ? (rec.marketSegment || "priority segment") : filters.segment.value;
  const choices = {
    software: {
      title: "Choose the workflow-execution promise PMM can substantiate",
      action: "Build one message test around setup speed, software usability, and instrument-to-informatics handoffs. Map every claim to dated proof, identify competitor counters, and hold field activation until three independent customer proof points are available.",
      decisionDeliverable: "A segment-specific messaging brief, substantiated claims matrix, competitor rebuttal, and proof-acquisition plan.",
    },
    "advanced-therapeutics": {
      title: `Choose the lead ${segment} workflow position and proof package`,
      action: "Select one lead workflow—oligo, LNP/RNA, MAM, or protein characterization—then write the target, reference class, value promise, points of parity/difference, and evidence required for a field-ready battlecard.",
      decisionDeliverable: "One governing positioning statement plus a battlecard and launch-message brief for the selected biopharma workflow.",
    },
    regulated: {
      title: "Approve the regulated-workflow message and substantiation plan",
      action: "Choose the regulated job Waters will own, define the method-readiness and compliance promise, quantify the proof gap versus the reference competitor, and route unsupported wording to a named study rather than the field.",
      decisionDeliverable: "A claims-safe regulated-workflow narrative, comparator proof matrix, and campaign activation gate.",
    },
    omics: {
      title: "Decide where Waters will defend versus differentiate in omics",
      action: "Choose the two omics workflows with the strongest strategic fit. For each, define the competitor frame, Waters point of difference, proof hierarchy, buyer-role message, and application-note gap.",
      decisionDeliverable: "An omics defense card and differentiated message architecture with explicit proof gaps.",
    },
    other: {
      title: "Turn the market signal into an approved message and seller-proof plan",
      action: "Translate the signal into a target buyer, buying situation, competitive alternative, value promise, and proof requirement. Do not activate until claim wording and source support are approved.",
      decisionDeliverable: "A positioning brief, approved-claims backlog, seller asset, and measurement plan.",
    },
  };
  const choice = choices[themeKey] || choices.other;
  const sourceRationale = rec.why || rec.urgency?.evidence || rec.title;
  return {
    ...rec,
    sourceDecisionTitle: rec.title,
    title: choice.title,
    why: `${sourceRationale} PMM must decide the buyer-facing promise, competitive frame, and proof threshold before activation.`,
    whyNow: `${sourceRationale} The marketing risk is launching a generic or unsupported message while competitors package the workflow more clearly.`,
    action: choice.action,
    decisionDeliverable: choice.decisionDeliverable,
    decisionOwners: "Product Marketing + Sales Enablement + Claims/Legal",
    tradeoff: "Narrower, substantiated positioning may reduce message breadth but improves field credibility and reuse.",
    falsifier: "Stop or reframe if independent customer evidence does not confirm the buying criterion or comparative proof cannot support the proposed wording.",
    urgency: rec.urgency ? {
      ...rec.urgency,
      decisionImplications: [
        `Message risk: ${sourceRationale}`,
        "Activation risk: no claim should reach sellers until the reference competitor, buyer role, and proof standard are explicit.",
      ],
      whyNowInsight: "Competitor activity is already shaping the buying narrative; PMM needs a governed response before field teams improvise one.",
    } : rec.urgency,
  };
}
...
function currentRecommendationSet(signals) {
  ...
  if (ranked.length) return state.view === "Marketing" ? ranked.map(marketingRecommendationFor) : ranked;
  ...
  const scoped = { ...generated, priorityBreakdown, roleFit: roleRelevanceScore(generated) };
  return [state.view === "Marketing" ? marketingRecommendationFor(scoped) : scoped];
}
...
function strategicThemeForRecommendation(recommendation) {
  ...
  const capabilitySentence = `${capability.charAt(0).toUpperCase()}${capability.slice(1)}`;
  if (state.view === "Marketing") {
    if (recommendation.regionalEvidenceMissing) return `Global evidence supports a ${capability} message hypothesis, but ${filters.geo.value}-specific buyer relevance is unconfirmed.`;
    if (["30d", "60d"].includes(filters.horizon.value)) return `${capabilitySentence} is a message-test priority, not yet an approved market position.`;
    if (filters.horizon.value === "90d") return `${capabilitySentence} is the quarter's highest-priority positioning and proof decision.`;
    if (["1y", "3y", "5y"].includes(filters.horizon.value)) return `${capabilitySentence} is persistent enough to require a clear defend-or-differentiate narrative.`;
    if (themeKey === "regulated") return "Regulated workflow proof is becoming a claims and campaign priority.";
    return `${capabilitySentence} requires a governed positioning choice and proof plan.`;
  }
  ...
  const items = [
    {
      ...
      action: marketingView
        ? `Message gate: use the signal to prioritize research, not as a market-demand claim. ${breakdown.confidenceState.guidance}`
        : `Time-window decision rule: ${context.decisionRule} ${breakdown.confidenceState.guidance}`,
    },
    {
      label: marketingView ? "Competitive narrative" : `${context.mode} competitor read`,
      headline: competitorRead.headline,
      detail: `${competitorRead.detail} ${competitorIntentForRecommendation(topRecommendation, signals)}`,
      action: marketingView
        ? `Action: compare ${competitorRead.competitor === "No clear competitor" ? "competitor" : competitorRead.competitor} claims with Waters' field-citable proof, then update the battlecard, objection handling, and message test.`
        : `Action: ${context.decisionRule} Compare ${competitorRead.competitor === "No clear competitor" ? "competitor" : competitorRead.competitor} claims against Waters proof points before the next roadmap review.`,
    },
    {
      label: marketingView ? "PMM activation gate" : "Waters decision gate",
  ...
  return `
    <article class="executive-decision-card">
      ...
      <dl class="executive-decision-facts">
        ...
        <div><dt>${state.view === "Marketing" ? "Next PMM Considerations" : "Next PM Considerations"}</dt><dd>${escapeHtml(deliverable)}</dd></div>
      </dl>
...
function marketingLeadershipHighlight(highlight) {
  const reads = {
    market: `PMM decision: determine whether ${highlight.title} is a priority segment message or only scientific context. Publication volume is directional; validate buyer urgency before using it as demand proof.`,
    competitor: `PMM action: update the ${highlight.title} battlecard with the competitor's exact new claims, the buying situation targeted, Waters' sourced response, and the comparative proof still missing.`,
    customer: `PMM action: convert this buyer friction into role-specific messaging and proof requirements. Treat the public evidence as directional unless multiple independent sources confirm the same buying criterion.`,
    corporate: `PMM implication: use this as evidence of competitor investment and narrative direction—not product superiority—and decide whether seller messaging or campaign emphasis must change.`,
    conference: `PMM action: use ${highlight.title} to test the priority narrative, capture competitor claims verbatim, and recruit proof or reference customers for the next campaign gate.`,
  };
  const labels = {
    market: "PMM market signal",
    competitor: "PMM competitive signal",
    customer: "PMM buyer signal",
    corporate: "PMM strategic signal",
    conference: "PMM activation opportunity",
  };
  return {
    ...highlight,
    label: labels[highlight.kind] || "PMM signal",
    detail: reads[highlight.kind] || highlight.detail,
  };
}
...
function leadershipBriefHighlights(signals) {
  ...
  return state.view === "Marketing" ? highlights.map(marketingLeadershipHighlight) : highlights;
}
...
function horizonTrendNarrative(candidate) {
  ...
  if (typeof state === "undefined" || state.view !== "Marketing") return baseNarrative;
  const theme = `${candidate.id || ""} ${candidate.title || ""}`.toLowerCase();
  if (/workflow experience|software|automation/.test(theme)) {
    return {
      ...baseNarrative,
      implication: "PMM should frame Next Gen LC around the customer job—reliable workflow execution from setup through review—not around a list of UHPLC specifications.",
      action: "Write one job-based category narrative, identify the workflow moments competitors claim to simplify, and build a message test with bench users plus QA/IT veto roles. Convert only validated advantages into claims or seller copy.",
    };
  }
  if (/modernization|serviceability|continuity|replacement/.test(theme)) {
    return {
      ...baseNarrative,
      implication: "The primary competitor is often inertia: keeping the validated method and avoiding migration risk. PMM must make continuity and risk reversal part of the value story.",
      action: "Build a replacement-positioning brief against keep-the-current-system, do nothing, and outsource alternatives. Specify method-transfer proof, service-continuity evidence, an evaluation offer, and the claim wording each buyer role can trust.",
    };
  }
  if (/biopharma|oligo|lnp|mam|therapeutic/.test(theme)) {
    return {
      ...baseNarrative,
      implication: "PMM needs one priority biopharma workflow position, not a broad claim that Waters serves every advanced-therapeutics use case equally well.",
      action: "Choose the lead workflow and segment, name the reference competitor, define the Waters point of difference and parity, then commission the missing application proof and build the vertical campaign around that single position.",
    };
  }
  if (/regulated|pfas|defensible/.test(theme)) {
    return {
      ...baseNarrative,
      implication: "Regulated buyers need defensible method execution, data integrity, and reproducibility proof; sensitivity language alone is insufficient and risky.",
      action: "Create a claims matrix for method readiness, compliance, reproducibility, and total workflow cost. Tag every statement proven, directional, or unsupported and assign the missing comparative study before campaign activation.",
    };
  }
  return {
    ...baseNarrative,
    implication: "PMM should decide where Waters will defend broad capability and where it can own a narrower, evidence-backed workflow position.",
    action: "Define the target segment, competitor frame, differentiated promise, buyer-role message, proof hierarchy, and application-note gap for the two highest-value workflows.",
  };
}
        ...
        return `
          <article class="overall-trend-card">
            ...
            <div class="trend-action">
              <span>${state.view === "Marketing" ? "Next PMM consideration" : "Next PM consideration"}</span>
...
function renderDecisionQueue(signals) {
  ...
  decisionQueue.innerHTML = decisions.length
    ? decisions
            ...
            <article class="decision-card">
              ...
              <dl class="decision-queue-facts">
                ${state.view === "Marketing"
                  ? `<div><dt>Next PMM Considerations</dt><dd>${escapeHtml(facts.deliverable)}</dd></div>`
        ...
        .join("")
    : `<div class="empty">No recommendation has enough public support under ${escapeHtml(filterScopeLabel())}. ${state.view === "Marketing" ? "Gather more customer, competitor, and application proof before activating a message or seller asset." : "Gather more conference, product-page, and customer evidence before changing the roadmap."}</div>`;
}
...
function competitorIntentProfile(competitor, signals) {
  ...
  const reportingContext = competitor === "PerkinElmer"
    ? currentEarningsSignals(state.data?.signals || [])
      .filter((signal) => inSelectedHorizon(signal.date))
      .find((signal) => signal.competitor === "Revvity, Inc.") || null
    : null;
  const filings = currentFilingInsights().filter((insight) => insight.competitor === competitor);
  const categorizedUrls = new Set([
    ...launches.map((item) => canonicalEvidenceUrl(timelineUrlForLaunch(item))),
    ...strategic.map((item) => canonicalEvidenceUrl(item.sourceUrl)),
    ...earnings.map((item) => canonicalEvidenceUrl(item.sourceUrl)),
  ].filter(Boolean));
  const newsroom = currentNewsroomSignals(signals)
    .filter((signal) => signal.competitor === competitor)
    .filter((signal) => !categorizedUrls.has(canonicalEvidenceUrl(signal.sourceUrl)));
  const profileCopy = {
    "Thermo Fisher": {
  ...
  const evidenceBits = [
    ...
    earnings.length ? `${earnings.length} earnings update${earnings.length === 1 ? "" : "s"}` : "",
    newsroom.length ? `${newsroom.length} newsroom update${newsroom.length === 1 ? "" : "s"}` : "",
    filings.length ? `${filings.length} filing insight${filings.length === 1 ? "" : "s"}` : "",
  ].filter(Boolean);
  const evidenceCount = launches.length + strategic.length + earnings.length + newsroom.length + filings.length;
  const evidenceItems = [
    ...launches.map((launch) => ({
      id: launch.id,
      type: "Launch",
    ...
    ...earnings.map((signal) => ({
      id: signal.id,
      type: /quarterly earnings result/i.test(signal.signalType || "") ? "Earnings result" : "Earnings announcement",
      title: signal.title,
      date: signal.date,
      sourceName: signal.sourceName,
      url: signal.sourceUrl,
...
function marketingCompetitorResponse(profile) {
  const competitorGuidance = {
    Agilent: {
      defend: "Equip sellers to defend method continuity, separations expertise, and regulated workflow confidence against Agilent's OpenLab, CrossLab, and regional-ecosystem story. Use sourced proof, not generic installed-base language.",
      differentiate: "Position Waters around defensible workflow execution for the priority application—method transfer, adsorption control, reproducibility, and LC-MS handoff—with one buyer-role proof point per claim.",
      accelerate: "Ship an Agilent battlecard, MAM/oligo message test, and proof-request brief; prioritize the missing comparative study before launching a broad workflow-superiority campaign.",
    },
    "Thermo Fisher": {
      defend: "Prevent Thermo's end-to-end portfolio breadth from defining the category. Give account teams a clear Waters workflow narrative with proof for separations, robustness, informatics, and application support.",
      differentiate: "Choose the biopharma workflow where Waters has the strongest application depth and evidence; message specialist execution and method confidence rather than matching portfolio breadth.",
      accelerate: "Create a vertical battlecard and campaign brief for the selected LNP, MAM, oligo, or protein workflow, with substantiated claims and named proof gaps.",
    },
    Shimadzu: {
      defend: "Counter a value-platform comparison with documented method continuity, service confidence, lifecycle cost, and regulated-workflow proof—not premium-brand assertions.",
      differentiate: "Frame Waters around lower adoption risk and defensible results for the target workflow; quantify only the labor, downtime, and transfer assumptions that can be validated.",
      accelerate: "Build a Shimadzu replacement battlecard, risk-reversal evaluation offer, and EVC model for routine QC before field activation.",
    },
    SCIEX: {
      defend: "Address SCIEX's instrument-plus-software workflow story with sourced proof for LC-to-MS handoffs, data review, compliance roles, and service ownership.",
      differentiate: "Position Waters where separations, sample-path control, and method continuity materially improve the complete analytical workflow; avoid unsupported MS-performance comparisons.",
      accelerate: "Create a joint LC-MS workflow proof plan and claims-safe rebuttal for the specific SCIEX system and buying situation in scope.",
    },
    PerkinElmer: {
      defend: "Use transparent service, uptime, method-transfer, and cost evidence for value-sensitive accounts; do not imply a broad PerkinElmer threat from sparse evidence.",
      differentiate: "Focus the Waters story on regulated chromatography depth and workflow confidence where the evidence supports it.",
      accelerate: "Continue proof collection before producing a broad campaign; prepare only a narrow replacement card for verified deal situations.",
    },
  };
  return competitorGuidance[profile.competitor] || {
    defend: "Protect active opportunities with sourced Waters proof against the competitor's exact claim.",
    differentiate: "Choose one buyer-relevant point of difference and map it to role-specific evidence.",
    accelerate: "Build the battlecard and proof plan before activating a campaign or seller claim.",
  };
}
...
function competitorIntentDetailMarkup(profile) {
  ...
  return `
    <article id="intent-selected-detail" class="intent-detail-panel risk-${profile.risk.toLowerCase()}" style="--intent-competitor-accent: ${escapeHtml(competitorAccent)}" role="tabpanel" aria-label="${escapeHtml(profile.competitor)} competitor intent">
      ...
      ${profile.reportingContext ? `
        <aside class="filing-earnings-boundary intent-reporting-boundary" aria-label="PerkinElmer and Revvity corporate reporting boundary">
          <strong>Public-reporting boundary</strong>
          <p>Current PerkinElmer is privately held and does not publish public quarterly earnings. Revvity is the former public PerkinElmer Life Sciences and Diagnostics company; its Q2 result is useful adjacent-market context, not a PerkinElmer financial result.</p>
          <a href="${escapeHtml(profile.reportingContext.sourceUrl)}" target="_blank" rel="noreferrer">${escapeHtml(profile.reportingContext.title)} ↗</a>
        </aside>
      ` : ""}
      ...
      <details class="intent-decision-detail">
        <summary>
          <span>${state.view === "Marketing" ? "Waters PMM Considerations" : "Waters PM Considerations"}</span>
          <small>Defend · differentiate · accelerate</small>
        </summary>
        <div class="intent-decision-detail-body">
          <div class="intent-impact-grid">
            <p><span>${state.view === "Marketing" ? "0–11 month message risk" : "0–11 month impact"}</span>${escapeHtml(shortTermImpact)}</p>
            <p><span>${state.view === "Marketing" ? "12–36 month positioning risk" : "12–36 month impact"}</span>${escapeHtml(longTermImpact)}</p>
          </div>
          <div class="intent-response-grid">
            <p><span>Defend</span>${escapeHtml(response.defend)}</p>
...
function marketingCustomerDecision(theme) {
  const decisions = {
    "method-transfer": "Approve the method-continuity message and comparative proof plan",
    "troubleshooting-recovery": "Define the reliability and recovery claim sellers can substantiate",
    "data-export-portability": "Choose the data-freedom position for QA, IT, and lab leadership",
    "workflow-setup": "Approve the ease-of-adoption message and evaluation offer",
  };
  return decisions[theme.key] || `Define the ${theme.label.toLowerCase()} message and proof threshold`;
}
...
function customerRoadmapInputsMarkup(items) {
  ...
  return `
    <section class="customer-roadmap-inputs" aria-label="${state.view === "Marketing" ? "Marketing Decision Inputs" : "Roadmap Decision Inputs"}">
      <div class="customer-roadmap-inputs-heading">
        <strong>${state.view === "Marketing" ? "Marketing Decision Inputs" : "Roadmap Decision Inputs"}</strong>
        <span>${state.view === "Marketing" ? "Buyer tension → proof required → PMM decision" : "Pain → validation step → decision"}</span>
      </div>
        ...
        ${rows.map((row) => {
          ...
          <article class="customer-roadmap-input">
            ...
            <div class="customer-roadmap-input-copy">
              <small>${state.view === "Marketing" ? "Buyer tension" : "Pain"}</small>
              <p>${escapeHtml(row.theme.pain)}</p>
            </div>
            <div class="customer-roadmap-input-copy">
              <small>${state.view === "Marketing" ? "Proof required" : "Validation step"}</small>
            ...
            <a class="customer-roadmap-decision-link" href="#decisions-needed" data-section-nav="decisions-needed">
              <small>${state.view === "Marketing" ? "PMM decision" : `Decision ${row.theme.roadmapDecision.number}`}</small>
              <strong>${escapeHtml(state.view === "Marketing" ? marketingCustomerDecision(row.theme) : row.theme.roadmapDecision.title)}</strong>
...
function marketingCustomerInsightAction(insight) {
  const text = `${insight.title || ""} ${insight.whatItMeans || ""} ${insight.pmAction || ""}`.toLowerCase();
  if (/method transfer|migration|validated/.test(text)) return "PMM consideration: lead with method continuity and migration-risk reduction; substantiate transfer time, rework, and acceptance criteria before making a comparative claim.";
  if (/reliab|uptime|service|troubleshoot|maintenance/.test(text)) return "PMM consideration: map reliability and recovery proof to the bench user, lab manager, and procurement case; avoid guaranteed-uptime language without controlled evidence.";
  if (/software|data|integrity|export|workflow/.test(text)) return "PMM consideration: create separate messages for analyst usability and QA/IT data-integrity risk, with exact software-version and workflow proof attached.";
  if (/cost|price|value|procurement/.test(text)) return "PMM consideration: build an EVC model against the customer's current alternative using transparent assumptions for labor, downtime, service, consumables, and method-transfer cost.";
  return "PMM consideration: convert the buyer language into a role-specific message hypothesis, identify the reference alternative, and require corroborated proof before field use.";
}
        ...
        .map((insight) => {
          ...
          return `
            <article class="voice-insight">
              ...
              <p class="question">${escapeHtml(state.view === "Marketing" ? marketingCustomerInsightAction(insight) : insight.pmAction)}</p>
...
function filingDisplayCompany(company) {
  const normalizedCompany = String(company || "").trim();
  return /^revvity(?:,\s*inc\.?)?$/i.test(normalizedCompany) ? "Revvity" : normalizedCompany;
}

function filingCorporateMovesForCompany(company) {
  const canonicalCompany = filingDisplayCompany(company);
  const matchingGroups = (state.filingInsights?.companyCorporateMoves || [])
    .filter((group) => filingDisplayCompany(group.competitor) === canonicalCompany);
  const seenItems = new Set();
  const items = matchingGroups.flatMap((group) => group.items || []).filter((item) => {
    const itemKey = `${item.sourceUrl || ""}|${item.name || ""}|${item.date || ""}`;
    if (seenItems.has(itemKey)) return false;
    seenItems.add(itemKey);
    return true;
  });
  const coverageNote = [...new Set(matchingGroups.map((group) => group.coverageNote).filter(Boolean))].join(" ");
  return { competitor: canonicalCompany, items, coverageNote };
}

function renderFilingInsights() {
  const insights = currentFilingInsights().map((insight) => ({
    ...insight,
  ...
  const requestedActiveCompetitor = filingDisplayCompany(state.activeFilingCompetitor);
  const activeCompetitor = sortedGroups.some(([competitor]) => competitor === requestedActiveCompetitor)
    ? requestedActiveCompetitor
    : sortedGroups[0]?.[0] || "";
  ...
  byId("filingInsights").innerHTML = activeGroup
    ? `
        <div class="intent-master-detail filing-master-detail">
          <nav class="intent-competitor-rail filing-company-rail" role="tablist" aria-label="Companies with earnings and filing evidence">
            ...
            ${sortedGroups.map(([competitor, companyInsights]) => {
              ...
              const corporateMoveItems = filingCorporateMovesForCompany(competitor).items;
        ...
        .map(([competitor, companyInsights]) => {
          const companyEarnings = earnings.filter((signal) => signal.competitor === competitor);
          const corporateMoves = filingCorporateMovesForCompany(competitor);
...
function renderPatentInsights() {
  ...
  const grouped = new Map();
  insights.forEach((insight) => {
    if (!grouped.has(insight.competitor)) grouped.set(insight.competitor, []);
    grouped.get(insight.competitor).push(insight);
  });
  const groups = [...grouped.entries()].sort((left, right) => {
    const leftRank = competitorOrder.indexOf(left[0]);
    const rightRank = competitorOrder.indexOf(right[0]);
    return (leftRank === -1 ? competitorOrder.length : leftRank) - (rightRank === -1 ? competitorOrder.length : rightRank)
      || left[0].localeCompare(right[0]);
  });
  const activeCompetitor = groups.some(([competitor]) => competitor === state.activePatentCompetitor)
    ? state.activePatentCompetitor
    : groups[0]?.[0] || "";
  state.activePatentCompetitor = activeCompetitor;
  const activeGroup = groups.find(([competitor]) => competitor === activeCompetitor);
  if (!activeGroup) {
    target.innerHTML = `
      <div class="patent-empty-state">
        <strong>No published patent applications match this view.</strong>
        <p>Try a longer horizon or broaden the competitor, market, or technology filter. No absence-of-investment conclusion is inferred from this coverage gap.</p>
      </div>
    `;
    return;
  }
  ...
  target.innerHTML = `
    <div class="intent-master-detail patent-master-detail">
      <nav class="intent-competitor-rail patent-company-rail" role="tablist" aria-label="Competitors with published patent applications">
        <div class="intent-rail-heading">Competitors</div>
        ${groups.map(([company, records]) => {
          const selected = company === competitor;
          const newest = records[0];
          return `
            <button type="button" class="intent-competitor-option${selected ? " is-selected" : ""}" data-patent-select="${escapeHtml(company)}" role="tab" aria-selected="${selected}" aria-controls="patent-selected-detail">
              <span class="intent-option-copy">
                <strong>${escapeHtml(company)}</strong>
                <span>${records.length} application${records.length === 1 ? "" : "s"}</span>
                <small>Latest publication ${formatDate(newest.publicationDate || newest.date)}</small>
              </span>
              <span class="intent-option-arrow" aria-hidden="true">›</span>
            </button>
          `;
      ...
      <section id="patent-selected-detail" class="patent-selected-detail" role="tabpanel" aria-label="${escapeHtml(competitor)} patent filing insights" style="--patent-accent:${escapeHtml(competitorColors[competitor] || "#087786")}">
        <header class="patent-detail-header">
          <div>
            <span>Patent activity readout</span>
...
function renderLeadershipBehaviorProfiles() {
  const target = byId("leadershipProfiles");
  const counter = byId("leadershipProfileCount");
  if (!target || !counter) return;
  const competitorOrder = ["Agilent", "Thermo Fisher", "Shimadzu", "SCIEX", "PerkinElmer"];
  ...
  state.activeLeadershipCompetitor = activeCompetitor;
  const profile = profiles.find((item) => item.competitor === activeCompetitor);
  if (!profile) {
    target.innerHTML = `
      <div class="leadership-profile-empty">
        <strong>No leadership profile matches this competitor filter.</strong>
        <p>Broaden the competitor filter to review the five current LC-MS leadership chains. No inference is made from an unavailable profile.</p>
      </div>
    `;
    return;
  }
  const people = leadershipPeopleForCompetitor(profile.competitor);
  const activePersonId = people.some((person) => person.id === state.activeLeadershipPersonByCompetitor[profile.competitor])
    ? state.activeLeadershipPersonByCompetitor[profile.competitor]
    : people[0]?.id || "";
  state.activeLeadershipPersonByCompetitor[profile.competitor] = activePersonId;
  ...
  target.innerHTML = `
    <div class="intent-master-detail leadership-master-detail">
      <nav class="intent-competitor-rail leadership-company-rail" role="tablist" aria-label="Competitor leadership profiles">
        <div class="intent-rail-heading">Competitors</div>
        ${profiles.map((item) => {
          const selected = item.competitor === profile.competitor;
          const portfolioLeader = item.leaders.find((leader) => /LC-MS/.test(leader.roleLabel));
          return `
      ...
      <section id="leadership-selected-detail" class="leadership-selected-detail" role="tabpanel" aria-label="${escapeHtml(profile.competitor)} leadership behavior profile" style="--leadership-accent:${escapeHtml(profile.accent || competitorColors[profile.competitor] || "#087786")}">
        <header class="leadership-detail-header">
          <div>
            <span>Observable leadership read</span>
            <h4>${escapeHtml(profile.competitor)}</h4>
            <p>Identity verified ${formatDate(state.leadershipProfiles?.asOfDate)} · LC-MS portfolio scope</p>
          </div>
          <div class="leadership-confidence" aria-label="Profile confidence">
            <span><b>${escapeHtml(profile.identityConfidence)}</b> identity confidence</span>
            <span><b>${escapeHtml(profile.behaviorReadConfidence)}</b> behavior read</span>
          </div>
        </header>
          ...
          <section class="leadership-person-slider" data-leadership-person-slider data-competitor="${escapeHtml(profile.competitor)}" tabindex="0" aria-label="${escapeHtml(profile.competitor)} leader profiles. Use left and right arrow keys to change person.">
            ...
            <header class="leadership-person-slider-header">
              ...
              <div class="leadership-person-slider-controls" aria-label="Change selected leader">
                ...
                <span>${activePersonIndex + 1} / ${people.length}</span>
                <button type="button" data-leadership-person-action="next" data-competitor="${escapeHtml(profile.competitor)}" aria-label="Show next ${escapeHtml(profile.competitor)} leader">→</button>
              </div>
            </header>
            <div class="leadership-person-stage" aria-live="polite">
              <article class="leadership-person-profile">
                <header class="leadership-person-profile-header">
                  <div>
                    <span>${escapeHtml(activeLeader.roleLabel)}</span>
                    <h5>${escapeHtml(activePerson.name)}</h5>
                    <strong>${escapeHtml(activeLeader.title)}</strong>
                    <p>${escapeHtml(activeLeader.scope)}</p>
                  </div>
                  <span class="leadership-person-status">${escapeHtml(activeLeader.status)}</span>
                </header>
...
function renderHiringPatterns() {
  const target = byId("hiringPatterns");
  const counter = byId("hiringPatternCount");
  if (!target || !counter) return;
  const competitorOrder = ["Agilent", "Thermo Fisher", "Shimadzu", "SCIEX", "PerkinElmer"];
  const profiles = currentHiringPatterns().sort((left, right) => (
    competitorOrder.indexOf(left.competitor) - competitorOrder.indexOf(right.competitor)
  ));
  counter.textContent = `${profiles.length} competitor pattern${profiles.length === 1 ? "" : "s"} · checked ${formatDate(state.hiringPatterns?.asOfDate)}`;
  const activeCompetitor = profiles.some((profile) => profile.competitor === state.activeHiringCompetitor)
    ? state.activeHiringCompetitor
    : profiles[0]?.competitor || "";
  state.activeHiringCompetitor = activeCompetitor;
  const profile = profiles.find((item) => item.competitor === activeCompetitor);
  if (!profile) {
    target.innerHTML = `<div class="hiring-pattern-empty"><strong>No hiring pattern matches this competitor filter.</strong><p>Broaden the competitor filter to compare public recruiting and talent-development signals.</p></div>`;
    return;
  }
  target.innerHTML = `
    <div class="intent-master-detail hiring-master-detail">
      <nav class="intent-competitor-rail hiring-company-rail" role="tablist" aria-label="Competitor hiring patterns">
        <div class="intent-rail-heading">Competitors</div>
        ${profiles.map((item) => {
          const selected = item.competitor === profile.competitor;
          return `
            <button type="button" class="intent-competitor-option${selected ? " is-selected" : ""}" data-hiring-competitor="${escapeHtml(item.competitor)}" role="tab" aria-selected="${selected}" aria-controls="hiring-selected-detail">
              <span class="intent-option-copy">
                <strong>${escapeHtml(item.competitor)}</strong>
                <span>${escapeHtml(item.aiTalentSignal)}</span>
                <small>${item.observations.length} public talent signals</small>
              </span>
              <span class="intent-option-arrow" aria-hidden="true">›</span>
            </button>
          `;
      ...
      <section id="hiring-selected-detail" class="hiring-selected-detail" role="tabpanel" aria-label="${escapeHtml(profile.competitor)} hiring pattern" style="--hiring-accent:${escapeHtml(profile.accent || competitorColors[profile.competitor] || "#087786")}">
        <header class="hiring-detail-header">
          <div>
            <span>Observed workforce signal</span>
            <h4>${escapeHtml(profile.competitor)}</h4>
            <p>${escapeHtml(profile.signalLabel)}</p>
          </div>
          ...
          <div class="hiring-skill-grid">
            ${profile.skillClusters.map((cluster) => `
              <article class="hiring-skill-card hiring-strength-${escapeHtml(cluster.strength.toLowerCase())}">
                <div><h6>${escapeHtml(cluster.name)}</h6><span>${escapeHtml(cluster.strength)}</span></div>
                <div class="hiring-strength-meter" role="img" aria-label="${escapeHtml(cluster.strength)} public hiring signal"><i></i><i></i><i></i></div>
        ...
        <div class="hiring-forward-grid">
          <section>
            <span>Likely Capability Build</span>
            <p>${escapeHtml(profile.likelyCapabilityBuild)}</p>
          </section>
          <section class="hiring-planning-implication">
            <span>Waters Planning Implication</span>
...
function renderStrategicSignals(signals) {
  const strategicSignals = currentCorporateNewsroomSignals(signals);
  ...
  byId("strategicSignalCount").textContent = strategicSignals.length > pageSize
    ...
    : `${strategicSignals.length} official update${strategicSignals.length === 1 ? "" : "s"}`;
  byId("strategicSignals").innerHTML = strategicSignals.length
    ? visibleSignals
        ...
        .join("")
    : `<div class="empty">No official newsroom, partnership, or corporate-move records match the selected filters.</div>`;
...
function setupStrategicPagination() {
  byId("strategicPagination").addEventListener("click", (event) => {
    ...
    renderStrategicSignals(competitorIntentSignals(currentSignals()));
...
function marketingApplicationTrendQuestion(trend, horizon) {
  const theme = trend.theme.toLowerCase();
  const timing = ["30d", "60d"].includes(horizon) ? "message test" : horizon === "90d" ? "quarterly campaign choice" : "positioning choice";
  if (theme.includes("lnp") || theme.includes("rna therapeutics")) return `Which LNP/RNA buyer and buying situation should PMM prioritize, and what carryover, compatibility, throughput, and LC-MS handoff proof is required before the ${timing}?`;
  if (theme.includes("oligonucleotide")) return `Should PMM lead with oligonucleotide method readiness, transfer confidence, or software-guided execution—and which competitor and proof standard define the ${timing}?`;
  if (theme.includes("pfas")) return `Which regulated PFAS job should Waters own, what compliance and reproducibility claims are defensible, and which reference competitor anchors the ${timing}?`;
  if (theme.includes("proteomics") || theme.includes("metabolomics")) return `Where should Waters defend parity versus claim differentiated application depth in omics, and which application note or customer proof is missing for the ${timing}?`;
  return `Which workflow-execution promise is most relevant to the buying committee, and what substantiated proof and competitor counter-position are needed for the ${timing}?`;
}
    ...
    ${trends.map(({ trend, signal }, index) => {
      ...
      return `
        <article class="trend-evidence-card">
          ...
          <p class="trend-pm-question"><strong>${state.view === "Marketing" ? "PMM question" : "PM question"}:</strong> ${escapeHtml(state.view === "Marketing" ? marketingApplicationTrendQuestion(trend, horizon) : applicationTrendQuestion(trend, horizon))}</p>
...
function renderSourceCounts(signals) {
  ...
  const patentInsights = currentPatentInsights();
  const leadershipProfiles = currentLeadershipProfiles();
  const hiringPatterns = currentHiringPatterns();
...
function updateRolePanelVisibility() {
  ...
  const patentPanel = byId("patent-filing-insights");
  const patentNav = byId("patentFilingNav");
  const leadershipProfilePanel = byId("leadership-behavior-profiles");
  const leadershipProfileNav = byId("leadershipBehaviorNav");
  const hiringPatternsPanel = byId("competitor-hiring-patterns");
  const hiringPatternsNav = byId("hiringPatternsNav");
  const refreshBlock = document.querySelector(".refresh-block");
  const hiddenForProductManagement = state.view === "Product" || state.view === "Marketing";
  if (refreshBlock) {
  ...
  appShell?.classList.remove("marketing-view");
  ...
  if (marketingWorkspace) {
    marketingWorkspace.hidden = true;
    marketingWorkspace.setAttribute("aria-hidden", "true");
  }
  if (standardNavigation) standardNavigation.hidden = false;
  if (marketingNavigation) marketingNavigation.hidden = true;
  if (navigationLabel) navigationLabel.textContent = "Roadmap Intelligence";
...
function render() {
  ...
  renderHiringPatterns();
  renderFilingInsights();
  renderStrategicSignals(competitorIntentSignals(signals));
  renderConferenceSources();
...
async function loadData() {
  ...
  const [
    ...
    patentInsightResponse,
    leadershipProfileResponse,
    leadershipPeopleResponse,
    hiringPatternResponse,
    ...
    fetch("data/patent_insights.json", { cache: "no-store" }),
    fetch("data/leadership_profiles.json", { cache: "no-store" }),
    fetch("data/leadership_people.json", { cache: "no-store" }),
    fetch("data/hiring_patterns.json", { cache: "no-store" }),
  ...
  if (!patentInsightResponse.ok) throw new Error(`Patent insight data load failed: ${patentInsightResponse.status}`);
  if (!leadershipProfileResponse.ok) throw new Error(`Leadership profile data load failed: ${leadershipProfileResponse.status}`);
  if (!leadershipPeopleResponse.ok) throw new Error(`Leadership people data load failed: ${leadershipPeopleResponse.status}`);
  if (!hiringPatternResponse.ok) throw new Error(`Hiring pattern data load failed: ${hiringPatternResponse.status}`);
  ...
  state.patentInsights = await governed(patentInsightResponse, "patent_insights");
  state.leadershipProfiles = await governed(leadershipProfileResponse, "leadership_profiles");
  state.leadershipPeople = await governed(leadershipPeopleResponse, "leadership_people");
  state.hiringPatterns = await governed(hiringPatternResponse, "hiring_patterns");
  ...
  setupPatentInsightNavigation();
  setupLeadershipProfileNavigation();
  setupHiringPatternNavigation();
