...
const watersCapabilityDefinitions = [
  { key: "method-transfer", label: "Method transfer", terms: ["method transfer", "method-transfer", "method continuity", "migration", "validated method", "compatibility", "equivalency"] },
  { key: "lc-platform", label: "LC platform", terms: ["lc platform", "hplc", "liquid chromatography", "lc system", "routine lc"] },
  { key: "uhplc-modules", label: "UHPLC modules", terms: ["uhplc", "uplc", "module", "detector", "pump"] },
  { key: "lc-ms-sensitivity", label: "LC-MS sensitivity", terms: ["lc-ms sensitivity"], pattern: /(?:(?:lc[- /]?ms|mass spectrom|qtof|\btof\b|triple quadrupole|triple quad|zenotof|xevo|lcms).{0,120}(?:sensitiv|signal[- ]to[- ]noise|limit of detection|low[- ]level)|(?:sensitiv|signal[- ]to[- ]noise|limit of detection|low[- ]level).{0,120}(?:lc[- /]?ms|mass spectrom|qtof|\btof\b|triple quadrupole|triple quad|zenotof|xevo|lcms))/i },
  { key: "lc-ms-ms-quantitation", label: "LC-MS/MS quantitation", terms: ["lc-ms/ms", "quantitation", "quantitative", "triple quadrupole", "triple quad"] },
  { key: "2d-lc", label: "2D LC", terms: ["2d lc", "2d-lc", "two-dimensional liquid chromatography"] },
  { key: "software-usability", label: "Software usability", terms: ["software usability", "ease of use", "data review", "setup", "training", "workflow friction"] },
  { key: "informatics", label: "Informatics / software", terms: ["informatics", "software", "data workflow", "data review", "digital lab", "empower", "chromeleon", "labsolutions", "sciex os"] },
  { key: "automation", label: "Automation", terms: ["automation", "automated", "autosampler", "plate loader", "sample handling", "walkaway", "workflow execution"] },
  { key: "application-kits", label: "Application kits", terms: ["application kit", "application workflow", "method package", "method readiness", "application note"] },
  { key: "sample-prep", label: "Sample prep", terms: ["sample prep", "sample preparation", "extraction", "cleanup"] },
  { key: "regulated-methods", label: "Regulated methods", terms: ["pfas", "regulated", "compliance", "validated method", "environmental contaminant", "audit trail", "data integrity"] },
  { key: "serviceability", label: "Serviceability / service", terms: ["serviceability", "service", "support", "repair", "maintenance", "parts", "diagnostic", "uptime", "downtime"] },
];
...
function setupCapabilityGapEvidenceLinks() {
  byId("featureGapMatrix").addEventListener("click", (event) => {
    ...
    const sourceMixLabel = entry.sourceMix.map((item) => `${item.label} ${item.count}`).join(" · ");
    byId("decisionEvidenceTitle").textContent = `${entry.capability}: Waters vs ${entry.competitor}`;
    byId("decisionEvidenceSummary").textContent = entry.records.length
      ? `${directionLabel} · ${entry.tier} · ${entry.sourceCount} dated source${entry.sourceCount === 1 ? "" : "s"} across ${entry.independentSourceCount} independent source ${entry.independentSourceCount === 1 ? "family" : "families"}. Source mix: ${sourceMixLabel}. ${entry.directionalFamilies.length ? `Directional basis: ${entry.directionalFamilies.join(", ")}.` : "No governed evidence family establishes a directional lead."} Color shows direction only.`
      : `No dated public source matches ${entry.capability} for Waters versus ${entry.competitor}. No direction is inferred.`;
    byId("decisionEvidenceList").innerHTML = entry.records.length
      ? entry.records.map((record) => `
          <article class="decision-evidence-card">
            <strong>${escapeHtml(record.title)}</strong>
            <span>${escapeHtml(gapMapEvidenceFamilyLabels[record.evidenceFamily] || "Public evidence")} · ${escapeHtml(record.evidenceType)} · ${escapeHtml(record.dateType)} ${escapeHtml(formatDate(record.date))}</span>
...
function headToHeadSource({ url, label, date, dateType = "Source date", sourceName = "Public source", confidence, independentSourceCount = 0 }) {
  return { url, label, date, dateType, sourceName, confidence, independentSourceCount: Number(independentSourceCount || 0) };
}

function headToHeadCustomerRecords(company, product) {
  return customerVoiceItemsForHorizon(filters.horizon.value, { ignoreCompetitor: true })
...
function headToHeadTechnicalEvidence(context) {
  const profile = (state.technicalComparisons?.profiles || []).find((item) => item.launchId === context.competitorProduct.id && (!item.watersId || item.watersId === context.waters.id));
  if (!profile) return { profile: null, rows: [] };
  return {
    profile,
    rows: (profile.rows || []).map((row) => ({
      row,
      watersSource: headToHeadSource({ url: row.watersSourceUrl, label: `${context.waters.product}: ${row.dimension}`, date: profile.asOfDate, dateType: "Evidence reviewed", sourceName: "Waters primary source" }),
      competitorSource: headToHeadSource({ url: row.competitorSourceUrl, label: `${context.competitorProduct.product}: ${row.dimension}`, date: profile.asOfDate, dateType: "Evidence reviewed", sourceName: `${context.match.competitor} primary source` }),
    })),
  };
}
...
function headToHeadClaimMarkup(claim) {
  return `<article class="pmm-h2h-claim" data-substantiation="${escapeHtml(claim.substantiation)}"><header><span class="pmm-h2h-badge pmm-h2h-${claim.substantiation.toLowerCase()}">${escapeHtml(claim.substantiation)}</span><small>${escapeHtml(claim.classification)}</small></header><p>${escapeHtml(claim.statement)}</p><div class="pmm-h2h-claim-meta"><span>${claim.datedSourceCount} dated source${claim.datedSourceCount === 1 ? "" : "s"}</span><span>${claim.independentSourceCount} established independent source organization${claim.independentSourceCount === 1 ? "" : "s"}</span><span>${claim.confidence == null ? "Confidence not established" : `Confidence ${claim.confidence}%`}</span><span>${claim.recency ? `Latest dated evidence ${formatDate(claim.recency)}` : "Recency not established"}</span><span>${escapeHtml(claim.approvalState || "Approval not established")}</span></div><div class="pmm-h2h-sources">${claim.sources.length ? claim.sources.map(headToHeadSourceMarkup).join("") : `<span class="pmm-unresolved">No source available</span>`}</div><details><summary>Evidence caveat</summary><p>${escapeHtml(claim.caveat)}</p></details></article>`;
}
...
function headToHeadPitchReasonMarkup(claim, index) {
  return `<article><span>${index + 1}</span><div><h5>${escapeHtml(claim.statement)}</h5><p>${escapeHtml(claim.classification)} · ${escapeHtml(claim.substantiation)} · ${escapeHtml(claim.approvalState)}</p>${claim.sources[0] ? `<div class="pmm-h2h-pitch-source">${headToHeadSourceMarkup(claim.sources[0])}</div>` : `<small class="pmm-unresolved">Proof source unresolved</small>`}</div></article>`;
}
    ...
    <div class="pmm-decision-grid">${decisions.map((decision, index) => `<article class="pmm-decision-card pmm-positioning-decision" data-positioning-rank="${index + 1}">
      ...
      <div class="pmm-decision-question-grid">
        ...
        <section>
          ...
          <div class="pmm-decision-proof"><div><strong>Applicable proof</strong>${decision.availableProof.length ? `<div class="pmm-evidence-reference-list">${decision.availableProof.map((proof) => pmmCanonicalEvidenceReferenceMarkup(proof, "Applicable proof")).join("")}</div>` : `<p>Proof unresolved — no evidence passed the governed compatibility checks.</p>`}${decision.rejectedProof?.length ? pmmCaveatDetailsMarkup(`${decision.rejectedProof.length} inapplicable proof record${decision.rejectedProof.length === 1 ? "" : "s"}`, ["These records are blocked as Inapplicable by governed compatibility checks. Review Claims at Risk for dimension-level mismatches."]) : ""}</div><div class="pmm-missing-proof"><strong>Missing study or approval</strong><p>${escapeHtml(decision.missingProof)}</p></div></div>
        </section>
        <section>
...
function renderMarketingClaimsProof(rows, visibleRows, governingPosition) {
  ...
  target.innerHTML = `
    <aside class="pmm-claim-risk-summary" aria-label="Highest-risk governed claim"><span>Highest Risk Under Current Filters</span><strong>${highestRiskClaim ? escapeHtml(highestRiskClaim.proposedClaimWording) : "No matching claim"}</strong><p>${highestRiskClaim ? `${escapeHtml(highestRiskClaim.substantiationStatus)} substantiation · ${escapeHtml(highestRiskClaim.approvalState)} · ${pmmClaimRiskScore(highestRiskClaim)} governed risk points` : "No unrelated claim was substituted."}</p></aside>
...
function pmmAppendixRecordMarkup(record) {
  ...
  return `<article class="pmm-appendix-record" ${evidenceId ? `id="pmm-evidence-object-${escapeHtml(evidenceId)}" data-canonical-evidence-id="${escapeHtml(evidenceId)}" tabindex="-1"` : ""}>
    <div><span>${evidenceId ? `${escapeHtml(evidenceId)} · ` : ""}${escapeHtml(record.type)}</span><strong>${escapeHtml(record.title)}</strong><small>${escapeHtml(record.sourceName)} · ${escapeHtml(date)} · ${escapeHtml(confidence)}${record.sourceDomain ? ` · ${escapeHtml(record.sourceDomain)}` : ""}</small>${pmmCaveatDetailsMarkup("Evidence summary and caveats", detailItems)}</div>
    ${sourceLink}
  </article>`;
}
...
function pmmStartHereModel(model) {
  ...
  return {
    ...
    governingStatus: model.governingPosition.approvalState,
    ...
    nextDecision: model.positioningDecisions[0] || null,
...
function renderMarketingStartHere(summary) {
  ...
  target.innerHTML = `<header class="pmm-command-header"><div><div class="pmm-eyebrow">Senior PMM Command Brief</div><h2 id="pmmStartHereTitle">What Waters Can Credibly Use to Win</h2><p>Customer-facing claims are separated from message hypotheses. Every usable direction has exact proof; every unsupported statement is visibly blocked.</p></div><div class="pmm-command-score"><strong>${summary.approvedClaimCount}</strong><span>approved claims available</span><small>${summary.defensibleDirectionCount} evidence-backed direction${summary.defensibleDirectionCount === 1 ? "" : "s"} still require approval</small></div></header>
    <div class="pmm-command-grid">
      ...
      <article class="pmm-command-card pmm-command-direction"><span>Best evidence-backed direction</span>${direction ? `<strong>${escapeHtml(direction.proposedClaimWording)}</strong><small>${escapeHtml(direction.substantiationStatus)} · ${escapeHtml(direction.approvalState)} · ${direction.independentSourceCount} established independent source organization${direction.independentSourceCount === 1 ? "" : "s"}</small><div class="pmm-command-sources">${directionSources.map((source) => pmmCanonicalEvidenceReferenceMarkup(source, "Applicable claim proof")).join("")}</div>` : `<strong>No applicable claim proof matches these filters.</strong><small>Use the battlecard discovery path and commission the required comparison.</small>`}</article>
      <article class="pmm-command-card pmm-command-blocked"><span>Do not say</span>${blockedClaim ? `<strong>${escapeHtml(blockedClaim.proposedClaimWording)}</strong><small>${escapeHtml(blockedClaim.substantiationStatus)} · ${escapeHtml(blockedClaim.comparabilityStatus)} · ${escapeHtml(blockedClaim.approvalState)}</small>` : `<strong>No governed claim row matches this target.</strong><small>Unrelated evidence was not substituted.</small>`}</article>
      <article class="pmm-command-card pmm-command-build"><span>Build this proof next</span><strong>${escapeHtml(direction?.nextRequiredAction || blockedClaim?.nextRequiredAction || decision?.missingProof || "Proof requirement unresolved")}</strong><small>Owner needed · Deadline needed · Success measure needed</small></article>
    </div>
    <div class="pmm-command-actions" aria-label="Product Marketing priority actions"><a href="#pmm-head-to-head" data-section-nav="pmm-head-to-head"><span>1</span><strong>Open ${summary.battlecardCount} product battlecard${summary.battlecardCount === 1 ? "" : "s"}</strong><small>${escapeHtml(summary.watersProduct)}</small></a><a href="#pmm-claims-risk" data-section-nav="pmm-claims-risk"><span>2</span><strong>Govern claims</strong><small>Proof compatibility and approval</small></a><a href="#pmm-positioning-decisions" data-section-nav="pmm-positioning-decisions"><span>3</span><strong>Fund the proof gap</strong><small>${decision ? escapeHtml(decision.activation) : "Decision unresolved"}</small></a><a href="#pmm-activation-artifacts" data-section-nav="pmm-activation-artifacts"><span>4</span><strong>Ship the asset</strong><small>${escapeHtml(summary.nearestDeadline.label)}</small></a></div>
    <dl class="pmm-command-context"><div><dt>Chosen segment</dt><dd>${escapeHtml(summary.chosenSegment)}</dd></div><div><dt>Waters product</dt><dd>${escapeHtml(summary.watersProduct)}</dd></div><div><dt>Governing position status</dt><dd>${escapeHtml(summary.governingStatus)}</dd></div><div><dt>Swing attribute</dt><dd>${escapeHtml(summary.swingAttribute)}</dd></div><div><dt>Competitor / inertia threat</dt><dd>${escapeHtml(summary.threat)}</dd></div><div><dt>Next required decision</dt><dd>${decision ? escapeHtml(decision.missingProof) : "Decision unavailable"}</dd></div><div><dt>Nearest activation deadline</dt><dd>${escapeHtml(summary.nearestDeadline.label)}</dd></div></dl>`;
}
...
function buildMarketingWorkspaceModel(signals) {
  ...
  let appendix = marketingEvidenceAppendixModel(signals);
  ...
  const baseModel = { contexts, governingPosition, marketChoice, buyingCommittee, adoptionValuePlans, positioningDecisions, claimRows, visibleClaimRows, narratives, activationActions, artifactProduction, breakReport, appendix, kpis };
...
function gapMapDirectionFromComparativeText(text, competitor) {
  ...
  if (/(?:no clear|not a useful differentiation|aligned|substantially overlap|cannot support (?:a )?winner|does not (?:establish|demonstrate)|not a demonstrated|no common-condition|conditions differ|determine whether|validate whether)/i.test(normalized)) return "parity";
  ...
  if (escapedCompetitor && new RegExp(`(?:${escapedCompetitor}|competitor).{0,120}(?:${favorable})`, "i").test(normalized)) return "behind";
  if (new RegExp(`waters.{0,120}(?:${favorable})`, "i").test(normalized)) return "ahead";
  if (/waters.{0,100}(?:inferior|behind|lower performance|slower)/i.test(normalized)) return "behind";
  if (escapedCompetitor && new RegExp(`(?:${escapedCompetitor}|competitor).{0,100}(?:inferior|behind|lower performance|slower)`, "i").test(normalized)) return "ahead";
  return null;
}

const gapMapEvidenceFamilyLabels = {
  "governed-comparison": "Governed comparisons",
  "official-launch": "Official launches",
  "capability-record": "Capability records",
  "customer-voice": "Customer voice",
};

function gapMapEvidenceRecord({ url, date, title, detail, evidenceType, evidenceFamily, direction = null, dateType = "Published" }) {
  const sourceDate = gapMapIsoDate(date);
  if (!isHttpUrl(url) || !sourceDate || sourceHealthForUrl(url) === "bad") return null;
  return { url, date: sourceDate, title, detail, evidenceType, evidenceFamily, direction, dateType };
}

function gapMapIndependenceKey(record) {
  if (record.evidenceType === "Public customer evidence") return canonicalEvidenceUrl(record.url);
  try {
    return new URL(record.url).hostname.toLowerCase().replace(/^www\./, "");
  } catch {
    return canonicalEvidenceUrl(record.url);
  }
}

function gapMapScopedLaunchesForCompetitor(competitor) {
  return (state.productData?.launches || [])
    .filter((launch) => launch.competitor === competitor)
    .filter((launch) => inSelectedHorizon(launch.date))
    .filter((launch) => geographyMatches(launch.geography))
    .filter((launch) => filters.segment.value === "All" || launch.marketSegment === filters.segment.value)
    .filter((launch) => filters.competitor.value === "All" || launch.competitor === filters.competitor.value);
}

function gapMapCustomerVoiceEvidence(definition, competitor) {
  const items = customerVoiceItemsForHorizon(filters.horizon.value, { ignoreCompetitor: true })
  ...
  const watersItems = items.filter((item) => item.company === "Waters");
  const competitorItems = items.filter((item) => item.company === competitor);
  const average = (records) => records.length ? records.reduce((sum, item) => sum + sentimentValue(item), 0) / records.length : null;
  const watersAverage = average(watersItems);
  const competitorAverage = average(competitorItems);
  const comparativeDirection = watersAverage == null || competitorAverage == null || watersAverage === competitorAverage
    ? null
    : watersAverage > competitorAverage ? "ahead" : "behind";
  return items.flatMap((item) => customerVoiceSourceLinks(item).map((link) => gapMapEvidenceRecord({
    url: link.url,
    date: link.sourceDate,
    title: `${item.company}: ${item.theme}`,
    detail: comparativeDirection
      ? `Public customer evidence is available for both Waters and ${competitor}; the directional vote compares the coded positive/mixed/negative signal for this capability.`
      : `This dated customer record establishes capability relevance, but does not establish a Waters-versus-${competitor} lead.`,
    evidenceType: "Public customer evidence",
    evidenceFamily: "customer-voice",
    direction: comparativeDirection,
    dateType: link.dateType || "Published",
  }))).filter(Boolean);
}

function gapMapTechnicalEvidence(definition, competitor) {
  const launches = state.productData?.launches || [];
  return (state.technicalComparisons?.profiles || []).flatMap((profile) => {
    const launch = launches.find((item) => item.id === profile.launchId);
    if (!launch || launch.competitor !== competitor || !inSelectedHorizon(launch.date)) return [];
    return (profile.rows || []).flatMap((row) => {
      const comparisonText = `${row.dimension} ${row.competitorValue} ${row.watersValue} ${row.interpretation}`;
      if (!gapMapCapabilityMatches(definition, comparisonText)) return [];
      const direction = gapMapDirectionFromComparativeText(comparisonText, competitor);
      const sourceDate = launch.date || profile.asOfDate;
      return [
        gapMapEvidenceRecord({
          url: row.competitorSourceUrl,
          date: sourceDate,
          title: `${competitor}: ${row.dimension}`,
          detail: row.interpretation,
          evidenceType: `Governed capability comparison · ${row.evidenceType || "conditions not coded"}`,
          evidenceFamily: "governed-comparison",
          direction,
        }),
        gapMapEvidenceRecord({
          url: row.watersSourceUrl,
          date: profile.asOfDate || sourceDate,
          title: `Waters: ${row.dimension}`,
          detail: row.interpretation,
          evidenceType: `Governed capability comparison · ${row.evidenceType || "conditions not coded"}`,
          evidenceFamily: "governed-comparison",
          direction,
          dateType: "Comparison record as of",
        }),
      ].filter(Boolean);
    });
  });
...
function gapMapLaunchComparisonEvidence(definition, competitor) {
  const launches = gapMapScopedLaunchesForCompetitor(competitor);
  const watersSystems = state.productComparisons?.watersSystems || [];
  return (state.productComparisons?.launchComparisons || []).flatMap((comparison) => {
    const launch = launches.find((item) => item.id === comparison.launchId);
    if (!launch) return [];
    const watersSystem = watersSystems.find((item) => item.id === comparison.closestWatersId);
    const contextText = `${comparison.impactRationale || ""} ${comparison.pmRead || ""} ${comparison.watersPositioning || ""}`;
    const matchingDimensions = (comparison.dimensions || []).filter((row) => gapMapCapabilityMatches(
      definition,
      `${row.dimension || ""} ${row.competitor || ""} ${row.waters || ""} ${row.pmRead || ""}`,
    ));
    if (!gapMapCapabilityMatches(definition, contextText) && !matchingDimensions.length) return [];
    const rowDirections = matchingDimensions
      .map((row) => gapMapDirectionFromComparativeText(`${row.dimension} ${row.competitor} ${row.waters} ${row.pmRead}`, competitor))
      .filter((value) => ["ahead", "behind"].includes(value));
    const rowDirectionScore = rowDirections.reduce((sum, value) => sum + (value === "ahead" ? 1 : -1), 0);
    const direction = rowDirectionScore > 0 ? "ahead" : rowDirectionScore < 0 ? "behind" : null;
    return [
      gapMapEvidenceRecord({
        url: pressReleaseUrlForLaunch(launch) || productPageUrlForLaunch(launch),
        date: launch.date,
        title: `${competitor}: ${launch.product}`,
        detail: direction ? comparison.pmRead : `The dated launch supports capability relevance; the loaded comparison does not establish a directional lead.`,
        evidenceType: "Governed launch comparison",
        evidenceFamily: "governed-comparison",
        direction,
      }),
      gapMapEvidenceRecord({
        url: watersSystem?.sourceUrl,
        date: watersSystem?.introducedYear ? `${watersSystem.introducedYear}-01-01` : "",
        title: `Waters: ${watersSystem?.product || "comparison baseline"}`,
        detail: direction ? comparison.watersPositioning : `Waters baseline for the governed launch comparison.`,
        evidenceType: "Governed launch comparison",
        evidenceFamily: "governed-comparison",
        direction,
        dateType: watersSystem?.dateBasis || "Introduction year",
      }),
    ].filter(Boolean);
  });
}

function gapMapCapabilityRecordEvidence(definition, competitor) {
  return (state.data?.recommendations || []).flatMap((recommendation) => {
    const capabilityText = `${recommendation.affectedCapability || ""} ${recommendation.action || ""}`;
    if (!gapMapCapabilityMatches(definition, capabilityText)) return [];
    return (recommendation.urgency?.competitorActions || [])
      .filter((action) => action.competitor?.startsWith(competitor))
      .map((action) => gapMapEvidenceRecord({
        url: action.sourceUrl,
        date: action.date,
        title: action.competitor,
        detail: action.decisionLink || action.pmKeyPoint || action.action,
        evidenceType: "Capability decision record",
        evidenceFamily: "capability-record",
        direction: gapMapDirectionFromComparativeText(`${action.pmKeyPoint} ${action.decisionLink}`, competitor),
      }))
      .filter(Boolean);
  });
}

function gapMapCell(definition, competitor) {
  const launchRecords = gapMapScopedLaunchesForCompetitor(competitor)
    .filter((launch) => gapMapCapabilityMatches(definition, `${launch.product} ${launch.technology} ${launch.signalType} ${launch.pmImplication} ${launch.roadmapQuestion}`))
    .map((launch) => gapMapEvidenceRecord({
      url: pressReleaseUrlForLaunch(launch) || productPageUrlForLaunch(launch),
      date: launch.date,
      title: `${competitor}: ${launch.product}`,
      detail: "This dated launch establishes capability activity, but a launch alone does not establish superiority over Waters.",
      evidenceType: "Official product launch",
      evidenceFamily: "official-launch",
    }))
    .filter(Boolean);
  const candidates = [
    ...gapMapCustomerVoiceEvidence(definition, competitor),
    ...gapMapTechnicalEvidence(definition, competitor),
    ...gapMapLaunchComparisonEvidence(definition, competitor),
    ...gapMapCapabilityRecordEvidence(definition, competitor),
    ...launchRecords,
  ];
  const byUrl = new Map();
  candidates.forEach((record) => {
    ...
    if (!existing || (!existing.direction && record.direction)) byUrl.set(key, record);
  });
  const records = [...byUrl.values()].sort((a, b) => new Date(b.date) - new Date(a.date));
  const familyVoteScores = records.reduce((scores, record) => {
    if (!["ahead", "behind"].includes(record.direction)) return scores;
    scores.set(record.evidenceFamily, (scores.get(record.evidenceFamily) || 0) + (record.direction === "ahead" ? 1 : -1));
    return scores;
  }, new Map());
  const familyVotes = [...familyVoteScores.entries()].map(([family, score]) => ({
    family,
    score: score > 0 ? 1 : score < 0 ? -1 : 0,
  })).filter(({ score }) => score !== 0);
  const governedDirectionalVotes = familyVotes.filter(({ family }) => family !== "customer-voice");
  const netDirection = familyVotes.reduce((sum, vote) => sum + vote.score, 0);
  // Customer voice is context, not a standalone comparative verdict. Ahead/Behind requires
  // at least one directional governed-comparison or capability-record family vote.
  const direction = !records.length
    ? "no-evidence"
    : !governedDirectionalVotes.length || netDirection === 0
      ? "parity"
      : netDirection > 0 ? "ahead" : "behind";
  const sourceCount = records.length;
  const independentSourceCount = new Set(records.map(gapMapIndependenceKey)).size;
  const tier = independentSourceCount >= 2 ? "Proven" : independentSourceCount === 1 ? "Directional" : "Insufficient";
  const sourceMix = Object.entries(gapMapEvidenceFamilyLabels).map(([family, label]) => ({
    family,
    label,
    count: records.filter((record) => record.evidenceFamily === family).length,
  })).filter(({ count }) => count > 0);
  return {
    key: `${definition.key}::${competitor}`,
    ...
    tier,
    sourceMix,
    directionalFamilies: governedDirectionalVotes.map(({ family }) => gapMapEvidenceFamilyLabels[family] || family),
    records,
  };
}

function buildCapabilityGapMap() {
  const selectedCompetitors = primaryCompetitors.filter((competitor) => filters.competitor.value === "All" || filters.competitor.value === competitor);
  const allCells = watersCapabilityDefinitions.flatMap((definition) => selectedCompetitors.map((competitor) => gapMapCell(definition, competitor)));
  const competitors = selectedCompetitors.filter((competitor) => allCells.some((cell) => cell.competitor === competitor && cell.sourceCount > 0));
  const capabilities = watersCapabilityDefinitions.filter((definition) => allCells.some((cell) => cell.capabilityKey === definition.key && competitors.includes(cell.competitor) && cell.sourceCount > 0));
  const cells = capabilities.flatMap((definition) => competitors.map((competitor) => allCells.find((cell) => cell.capabilityKey === definition.key && cell.competitor === competitor)));
  state.capabilityGapEvidence = cells;
  return {
    competitors,
    capabilities,
    cells,
    hiddenCompetitors: selectedCompetitors.filter((competitor) => !competitors.includes(competitor)),
...
function gapMapCellMarkup(cell) {
  ...
  return `
    <button type="button" class="gap-map-cell gap-map-${cell.direction}" data-gap-evidence-key="${escapeHtml(cell.key)}" role="cell" aria-label="Waters versus ${escapeHtml(cell.competitor)} on ${escapeHtml(cell.capability)}: ${label}; ${cell.sourceCount} dated source${cell.sourceCount === 1 ? "" : "s"}; ${cell.tier}">
      <strong>${label}</strong>
      <span class="gap-map-confidence"><b>${cell.sourceCount}</b><span>${cell.tier}</span></span>
      <small>${cell.sourceMix.length} source ${cell.sourceMix.length === 1 ? "type" : "types"} · view</small>
    </button>
  `;
}
...
function renderFeatureGapMatrix() {
  ...
  const noteItems = [
    model.hiddenCompetitors.length ? `${model.hiddenCompetitors.join(", ")} ${model.hiddenCompetitors.length === 1 ? "has" : "have"} no dated evidence in the active scope` : "",
    model.hiddenCapabilities.length ? `${model.hiddenCapabilities.length} capability row${model.hiddenCapabilities.length === 1 ? " has" : "s have"} no dated competitor evidence` : "",
  ].filter(Boolean);
  byId("featureGapMatrix").innerHTML = `
    <div class="gap-map-guide">
      <strong>Read across:</strong> each cell is Waters versus that named competitor. <b>Red = Waters is behind this competitor here.</b>
    </div>
    <div class="gap-grid-scroll" tabindex="0" aria-label="Scrollable competitor capability heatmap">
      ${model.competitors.length && model.capabilities.length ? `
        <div class="gap-map-grid" role="table" aria-label="Directional Waters competitive capability gap map" style="--gap-columns:${model.competitors.length}">
          <div class="gap-map-corner" role="columnheader">Waters capability</div>
          ${model.competitors.map((competitor) => `<strong class="gap-map-competitor" role="columnheader">${escapeHtml(competitor)}</strong>`).join("")}
          ${model.capabilities.map((definition) => `
            <strong class="gap-map-capability" role="rowheader">${escapeHtml(definition.label)}</strong>
            ${model.competitors.map((competitor) => gapMapCellMarkup(model.cells.find((cell) => cell.capabilityKey === definition.key && cell.competitor === competitor))).join("")}
          `).join("")}
        </div>
      ` : `<div class="empty"><strong>Not enough evidence yet</strong><span>No dated public capability evidence matches the active filters.</span></div>`}
    </div>
    ...
    <div class="gap-map-confidence-legend"><b>Evidence strength:</b> badge = dated source count · Proven ≥2 independent source families · Directional 1 · Insufficient 0. Evidence can include governed comparisons, capability records, official launches, and customer voice. Color never encodes confidence.</div>
    ...
    <details class="gap-map-method">
      <summary>How evidence becomes Ahead / Parity / Behind</summary>
      <p>Evidence is filtered to one Waters capability and one competitor, then grouped into governed comparisons, capability records, official launches, and customer voice. Each family gets at most one directional vote, so a high volume of Reddit or review records cannot overpower a governed comparison. Customer voice contributes context only when both vendors have matching dated records and can never independently create Ahead or Behind. Governed technical, launch-comparison, or capability records contribute only when their matched comparative text explicitly identifies a stronger side. Official launches establish capability activity but not superiority. Dated one-sided or non-comparative evidence contributes Parity. Zero dated sources always = No evidence.</p>
    </details>
  `;
}
